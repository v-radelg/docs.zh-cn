...
## <a name="task-status"></a>任务状态  
 ...
 所有其他任务在热状态下开始其生命周期，这意味着它们表示的异步操作已启动，并且其任务状态是 <xref:System.Threading.Tasks.TaskStatus.Created?displayProperty=nameWithType> 以外的枚举值。 从 TAP 方法返回的所有任务必须在方法内部被激活。 **如果 TAP 方法内部使用任务的构造函数来实例化一个任务，那么在返回这个任务对象之前必须对这个任务调用<xref:System.Threading.Tasks.Task>类的<xref:System.Threading.Tasks.Task.Start%2A>方法。** TAP 方法的使用者可以安全地假设返回的任务处于活动状态且不应尝试对从 TAP 方法返回的任何 <xref:System.Threading.Tasks.Task.Start%2A> 调用 <xref:System.Threading.Tasks.Task>。 对活动的任务调用 <xref:System.Threading.Tasks.Task.Start%2A> 将引发 <xref:System.InvalidOperationException> 异常。  
